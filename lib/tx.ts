...
export const waitReceipt = async (web3: Web3, txHash: string): Promise<TransactionReceipt> => {
  for (let i = 0; i < 50; i += 1) {
    ...
    if (!receipt.status) {
      ...
      throw createSdkError(IWalletProviderErrorCode.TransactionFailed, 'Transaction has failed');
    }

    return receipt;
  }

  throw createSdkError(IWalletProviderErrorCode.GetReceiptFailed, 'Failed to get receipt after 50 retries');
};
